...
In principle, there might be advantages to modelling sampling error and measurement error simultaneously. However, doing so would require extra assumptions about the nature of the measurement errors, and the ways that these interacted with sample size. This would add extra complexity.

In practice, a simpler approach, where `smooth_prob()` is applied to deal with sampling error, and then `scale_prob()` is applied to deal with measurement error, is probably sufficient. 
