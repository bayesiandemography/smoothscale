...
- `prob_report` is the original probability estimate for each area
- `prob_target` is an overall probability, assumed to be error-free
- `wt` is an optional weighting variable, used to calculate an overall probability from `prob_report`.
...
scaled_agesex <- smoothed_agesex |>
  ...
  group_by(age, sex) |>
  mutate(scaled = scale_prob(prob_report = smoothed,
