...
test_that("'scale_prob' works when scaling up", {
  ...
  prob_unscaled <- x / size
  ans <- scale_prob(prob_unscaled = prob_unscaled, prob_target = prob_true)
  expect_equal(mean(ans), prob_true, tolerance = 0.01)
  ans_wt <- scale_prob(prob_unscaled = prob_unscaled, prob_target = prob_true, wt = size)
...
test_that("'scale_prob' works when scaling down", {
  ...
  x <- rbinom(n = 100, size = size, prob = scale * prob_true)
  prob_unscaled <- x / size
  ans <- scale_prob(prob_unscaled, prob_target = prob_true, wt = size)
